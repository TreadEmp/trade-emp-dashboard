...
export default function JobCategoryList({
  ...
  const onDelete = (id) => {
    Axios.baseInstance({
      method: "DELETE",
      url: `/job-categories?jobCategoryId=${id}`,
    })
      .then((res) => {
        setMessage("Requested job category was deleted successfully");
        setSeverity("success");
        setShowAlert(() => {
          return true;
        });
      ...
      .catch((e) => {
        setMessage("Requested job category could not be deleted");
        setSeverity("error");
        setShowAlert(() => {
          return true;
        });
  ...
  const onView = (id) => {
    router.push(`/jobCategories/view?id=${id}`);
  };

  const onEdit = (id) => {
    router.push(`/jobCategories/${id}`);
  };
        ...
        <div className="rounded-t mb-0 px-4 py-3 border-0">
          <div className="flex flex-wrap items-center">
            ...
            <div className="text-center flex justify-between">
              <Link href="/jobCategories/new" passHref>
                <button

...
export default function ExperiencesList({
  ...
  const onDelete = (id) => {
    Axios.baseInstance({
      method: "DELETE",
      url: `/experiences?experienceId=${id}`,
    })
      .then((res) => {
        setMessage("Requested experience was deleted successfully");
        setSeverity("success");
        setShowAlert(() => {
          return true;
        });
      ...
      .catch((e) => {
        setMessage("Requested experience could not be deleted");
        setSeverity("error");
        setShowAlert(() => {
          return true;
        });
  ...
  const onView = (id) => {
    router.push(`/experiences/view?id=${id}`);
  };
        ...
        <div className="rounded-t mb-0 px-4 py-3 border-0">
          <div className="flex flex-wrap items-center">
            ...
            <div className="text-center flex justify-between">
              <Link href="/experiences/new" passHref>
                <button
                ...
                return (
                  <tr key={experience.id}>
                    ...
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      {categories.map((cat) => {
                        if (experience.category === cat.value) {
                          return cat.label;
                        }
                      })}
                    </td>
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      <Switch
                        checked={experience.isDisplayInApp}
                        onChange={() => onToggle(experience.id, index)}
